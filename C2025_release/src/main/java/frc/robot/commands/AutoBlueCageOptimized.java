...
public class AutoBlueCageOptimized extends SequentialCommandGroup {
  /** Creates a new AutoBlueCageOptimized. */
  public AutoBlueCageOptimized() {
    ...
    addCommands(
      ...
      RobotContainer.runTrajectoryPathPlannerWithForceResetOfStartingPose("Blu-Reef11ToCoralTop", false, false)
        .alongWith(
          new ElevatorAllTheWayDown().andThen(new AutoIntakeSequenceCoralToPan())
        ),
      RobotContainer.runTrajectoryPathPlannerWithForceResetOfStartingPose("Blu-CoralTopToReef9", false, false)
        .alongWith(
          (new WaitCommand(0.5).alongWith(new IntakeCoralPanToFinal()))
            .andThen(
              new TeleopMoveToL4RotateArm()
            )
        ),
