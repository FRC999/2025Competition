...
public class RedRishikaTest extends SequentialCommandGroup {
  /** Creates a new AutoBlu2CoralVision. */
  public RedRishikaTest() {
        ...
        new DeferredCommand(
            () -> new SetOdometryToVisionPose()
                .andThen(new PrintCommand(
                    "---A From: " + RobotContainer.driveSubsystem.getInitialVisionAidedOdometryPose( new Pose2d(14.524, 1.352, Rotation2d.k180deg)) +
                        " To: " + RobotPoseConstants.visionRobotPoses.get("RobotRedReef6Right").toString())),
            Set.of()),
        new DeferredCommand(
            () -> RobotContainer.runTrajectory2PosesSlow(
              RobotContainer.driveSubsystem.getInitialVisionAidedOdometryPose( new Pose2d(14.524, 1.352, Rotation2d.k180deg)), // if vision is not available at the start, use that pose
