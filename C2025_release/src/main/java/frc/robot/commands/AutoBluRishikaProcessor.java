...
public class AutoBluRishikaProcessor extends SequentialCommandGroup {
  /** Creates a new AutoRed2CoralVisionRishika. */
  public AutoBluRishikaProcessor() {
      ...
      new DeferredCommand(
          () -> new SetOdometryToVisionPose()
              .andThen(new PrintCommand(
                  "---A From: " + RobotContainer.driveSubsystem.getInitialVisionAidedOdometryPose( new Pose2d(7.200, 1.910, Rotation2d.k180deg)) +
                      " To: " + RobotPoseConstants.visionRobotPoses.get("RobotBluReef5Right").toString())),
          Set.of()),
      new DeferredCommand(
          () -> RobotContainer.runTrajectory2PosesSlow(
              RobotContainer.driveSubsystem.getInitialVisionAidedOdometryPose( new Pose2d(7.200, 1.910, Rotation2d.k180deg)), // if vision is not available at the start, use that pose
