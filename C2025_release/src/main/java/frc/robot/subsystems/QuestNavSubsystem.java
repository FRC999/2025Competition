...
public class QuestNavSubsystem extends SubsystemBase {
  ...
  private double poseChangeTolerance = 0.5; // We assume we will not have a difference more than 0.5m between where the chassis is vs where Quest tells it it is in
  ...
  public Command offsetTranslationCharacterizationCommand() {
    double rotationalSpeed = SwerveChassis.MaxAngularRate / 6.0;
  ...
  public Command offsetAngleCharacterizationCommand() {
    double driveSpeed = SwerveChassis.MaxSpeed / 6.0;
  ...
  public void periodic() {
    ...
    if (questNav.isTracking()) {
      ...
      if(initialPoseSet && SwerveChassis.CTR_ODOMETRY_UPDATE_FROM_QUEST && !isCharacterizationRunning) {
